...
export default function EpisodesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [sceneCandidates, setSceneCandidates] = useState<Record<string, { blob: Blob; sourceHash: string; reviewed?: boolean; review?: SceneReview }>>({});
  ...
  const generateScene = async (cutIdx: number, revision?: string) => {
    if (!project) return;
    const episode = episodes[activeEp];
    const cut = episode?.cuts[cutIdx];
    ...
    try {
      const result = await requestSceneImage(currentProject, episode, cut, "direct", "finish", revision, sceneCandidates[cut.id]?.sourceHash === sceneHash(currentProject, episode, cut) ? sceneCandidates[cut.id]?.blob : undefined);
      setSceneCandidates((current) => ({ ...current, [cut.id]: { blob: result.blob, sourceHash: result.sourceHash, review: result.review, reviewed: false } }));
    } catch (error) {
  ...
  const acceptScene = async (cutIdx: number) => {
    ...
    if (!candidate) return;
    if (!canApplyScene(candidate.review, candidate.reviewed)) {
      setVisualError(sceneReviewSummary(candidate.review));
      return;
    }
    ...
    try {
      ...
      const next = episodes.map((episode, episodeIndex) => episodeIndex === activeEp
        ? { ...episode, cuts: (episode.cuts ?? []).map((item, index) => index === cutIdx ? { ...item, sceneImageAssetId: asset.id, sceneSourceHash: candidate.sourceHash, sceneReview: candidate.review } : item) }
        : episode);
      setEpisodes(next);
      updateProject(id, { episodes: next });
                        ...
                        <div className="col-span-2 rounded-xl border border-[#E4DDF8] bg-[#FAF8FF] p-3 sm:p-4 space-y-3">
                          ...
                          {sceneCandidates[cut.id] && (
                            <div className="rounded-xl border border-[#C4B5FD] bg-white p-3 grid gap-3 sm:grid-cols-[180px_1fr]">
                              ...
                              <div className="self-center">
                                ...
                                <p className="mb-2 text-[11px]">{sceneReviewSummary(sceneCandidates[cut.id].review)}</p>
                                {sceneCandidates[cut.id].review?.issues.map((issue, index) => <p key={index} className="mb-1 text-[11px] text-orange-800">{issue}</p>)}
                                <label className="mb-3 flex items-start gap-2 text-[11px]"><input type="checkbox" checked={Boolean(sceneCandidates[cut.id].reviewed)} disabled={sceneCandidates[cut.id].review?.anatomy?.status !== "pass" || sceneCandidates[cut.id].review?.status !== "checked"} onChange={event => { const reviewed = event.target.checked; setSceneCandidates(current => ({ ...current, [cut.id]: { ...current[cut.id], reviewed } })); }} /> 구도·표현의 검수 안내를 확인했습니다. 신체 오류와 검수 미완료는 직접 확인으로 적용할 수 없습니다.</label>
                                <div className="flex gap-2">
                                  <button type="button" disabled={!canApplyScene(sceneCandidates[cut.id].review, sceneCandidates[cut.id].reviewed)} onClick={() => acceptScene(cutIdx)} className="inline-flex items-center gap-1 rounded-full bg-[#7C3AED] text-white px-3 py-1.5 text-[11px] font-semibold"><Check className="w-3 h-3" /> 적용</button>
                          ...
                          {cut.storyboard ? (
                            ...
                            <StoryboardEditor
                              ...
                              sceneReview={sceneCandidates[cut.id] ? sceneCandidates[cut.id].review : cut.sceneReview}
                              ...
                              onGenerateScene={(revision) => generateScene(cutIdx, revision)}
                            />
                            </>
                          ) : (
