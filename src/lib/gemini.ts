...
export async function generateSceneImage(input: {
  ...
  const task = input.stage === "sketch"
    ? "DRAW a complete PEN-LINE STORYBOARD scene using reference image 1 and the edited control map. Existing artwork is a composition reference, not separate pieces to paste. Keep all people, hands and connected objects in a single coherent perspective. Draw the environment visibly rather than writing its name. Never add dialogue, balloons, labels or guides."
  ...
  const stageDirection = input.stage === "sketch"
    ? "STORYBOARD PEN-LINE STAGE — LINE ART ONLY, NEVER FINISHED ART: Draw ONE coherent monochrome production storyboard, including ALL people, their hands, contact objects and environment together in a single perspective. Use readable black or graphite-gray contour lines on plain white, with economical interior lines for faces, clothing folds, props and background perspective. Keep the acting and silhouettes clear. Do NOT add flat color, gray fills, hatching, screentones, gradients, cast shadows, lighting, texture, particles, speed lines, glow, atmosphere or other finishing effects. This is a composition-and-pose check at the PEN-LINE step, not a rough scribble and not a finished illustration. This is one art region of a vertical-scroll webtoon, never a four-panel comic, page grid or contact sheet. Draw whole connected objects: laptop screen/hinge/keyboard must connect; tablet front/back/stand must be physically consistent; hands must contact the correct surface. Do not separate people or props onto independent canvases. White paper and monochrome lines only; no color, green-screen or transparency."
    : "WEBTOON FINAL RENDER STAGE — PUBLICATION-READY FINISH: Turn the approved full-scene pen-line storyboard into a polished final webtoon panel. Preserve object topology, visible front/back, hand contacts, laptop screen/hinge/keyboard and tablet/stand orientation. Apply complete character and environment colors, material-specific rendering, controlled line cleanup, ambient and directional light, form shadows, cast shadows, contact shadows and depth separation. Add scene-appropriate finishing details when they support the described action and mood: atmospheric perspective, rim light, bloom or glow, rain, dust, smoke, particles, speed lines, impact accents, reflected light and subtle color grading. Effects must be motivated by the scene rather than added decoratively, must not hide faces, hands or interaction props, and must remain consistent with the project art direction. The result must look like the final post-coloring, background/effects and finishing stage of a serialized webtoon, not flat colors or an intermediate draft. Do not redesign the scene or complete clipped objects by changing camera framing. This is one art region within a vertical-scroll webtoon, not a page grid.";
...
- Art direction: ${input.stage === "sketch" ? "Monochrome webtoon storyboard pen lines on white. Preserve the project's character design language, but omit every coloring, shading, texture and effects pass." : `${artStyle(input.context)}. Fully rendered final webtoon art with completed coloring, background, lighting, shadows, materials, depth and scene-motivated effects.`}
