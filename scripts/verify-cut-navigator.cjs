...
assert.ok(tree.props.className.includes('sticky top-16'), 'navigation remains below the page header while scrolling');
assert.ok(tree.props.className.includes('bg-[#FBF9F6]'), 'opaque outer strip masks artwork around the rounded toolbar');
assert.ok(all(tree, n => n.props?.className?.split(' ').includes('bg-white')).length, 'toolbar background is opaque');
...
assert.ok(page.indexOf('<CutNavigator') > page.indexOf('{/* 제목 & 줄거리 */}'));
assert.ok(/<CutNavigator[^>]*\/>\s*\{\/\* 콘티 \*\/\}/.test(page), 'navigation sits immediately above cut composition');
...
console.log('PASS: 12+ cuts, stable ID navigation, AI append, deletion, episode switching, empty state and opaque sticky navigation above cut composition');
